...
class _HomeFormAmministrazioneNewPageState
  ...
  Future<void> checkTickets() async{
    try{
      http.Response response = await http.get(Uri.parse('$ipaddress2/api/ticket'));
      if(response.statusCode == 200){
  ...
  Future<void> getAllOrdini() async{
    try{
      var apiUrl = Uri.parse('$ipaddress2/api/ordine');
      var response = await http.get(apiUrl);
      if(response.statusCode == 200){
        var jsonData = jsonDecode(utf8.decode(response.bodyBytes));
  ...
  Future<List<NotaTecnicoModel>> getNote() async {
    try {
      http.Response response = await http.get(
        Uri.parse('$ipaddress2/api/noteTecnico/ordered'),
      );
