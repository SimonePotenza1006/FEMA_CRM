...
class _DettaglioSpesaFornitorePageState extends State<DettaglioSpesaFornitorePage>{
  ...
  Future<List<Uint8List>> fetchImages() async{
    final url = '$ipaddress2/api/immagine/movimenti/${int.parse(widget.movimento.id.toString())}/images';
      ...
      for(var image in pickedImages){
        if(image.path.isNotEmpty){
          ...
          var request = http.MultipartRequest(
            'POST',
            Uri.parse('$ipaddress2/api/immagine/movimento/${int.parse(widget.movimento.id!.toString())}'),
          );
          request.files.add(
