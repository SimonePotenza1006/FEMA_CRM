...
class _DettaglioSopralluogoPageState extends State<DettaglioSopralluogoPage> {
  ...
  Future<List<Uint8List>> fetchImages() async {
    final url = '$ipaddress2/api/immagine/sopralluogo/${int.parse(widget.sopralluogo.id.toString())}/images';
      ...
      for (var image in pickedImages)  {
        if (image.path != null && image.path.isNotEmpty) {
          ...
          var request = http.MultipartRequest(
            'POST',
            Uri.parse('$ipaddress2/api/immagine/sopralluogo/${sopralluogo}'),
          );
          request.files.add(
