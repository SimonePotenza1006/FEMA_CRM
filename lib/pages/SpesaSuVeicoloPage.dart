...
class _SpesaSuVeicoloPageState extends State<SpesaSuVeicoloPage> {
  ...
  Future<http.Response?> saveSpesa() async {
    ...
    try {
      response = await http.post(
        Uri.parse('$ipaddressProva2/api/spesaVeicolo'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'data': DateTime.now().toIso8601String(),
  ...
  Future<void> getAllSpese() async{
    try{
      var apiUrl = Uri.parse('$ipaddressProva2/api/spesaVeicolo/ordered');
      ...
      try {
        ...
        for (var foto in pickedImages) {
          var request = http.MultipartRequest(
            'POST',
            Uri.parse('$ipaddressProva2/api/immagine/spesa/${int.parse(
  ...
  Future<void> getTipologieSpesa() async {
    try {
      var apiUrl = Uri.parse('$ipaddressProva2/api/tipologiaSpesaVeicolo');
      var response = await http.get(apiUrl);
      if (response.statusCode == 200) {
        var jsonData = jsonDecode(utf8.decode(response.bodyBytes));
