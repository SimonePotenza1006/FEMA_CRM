  ...
  Future<void> getAllTask() async {
    ...
    try {
      ...
      var apiUrl = (widget.utente.cognome! == "Mazzei" || widget.utente.cognome! == "Chiriatti")
          ? Uri.parse('$ipaddressProva2/api/task/allattivi')
          : Uri.parse('$ipaddressProva2/api/task/utente/' + widget.utente!.id!);
...
class TaskDataSource extends DataGridSource{
  ...
  Future<void> getAllTaskNO() async{
    try{
      var apiUrl = (utente.cognome! == "Mazzei" || utente.cognome! == "Chiriatti") ? Uri.parse('$ipaddressProva2/api/task/allattivi')
          : Uri.parse('$ipaddressProva2/api/task/utente/'+utente.id!);
      var response = await http.get(apiUrl);
      if(response.statusCode == 200){
        var jsonData = jsonDecode(utf8.decode(response.bodyBytes));
        List<TaskModel> commissioni = [];
