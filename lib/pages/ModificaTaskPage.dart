...
class _ModificaTaskPageState
      ...
      for(var image in pickedImages){
        if(image.path.isNotEmpty){
          var request = http.MultipartRequest(
            'POST',
            Uri.parse('$ipaddressProva2/api/immagine/task/${int.parse(widget.task.id.toString())}'),
          );
          request.files.add(
  ...
  Future<ap.AudioPlayer> fetchAudio() async {
    ...
    final url = '$ipaddressProva2/api/immagine/task/${int.parse(widget.task.id.toString())}/audio';
  ...
  Future<List<Uint8List>> fetchImages() async {
    final url = '$ipaddressProva2/api/immagine/task/${int.parse(widget.task.id.toString())}/images';
    http.Response? response;
    try {
      response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
