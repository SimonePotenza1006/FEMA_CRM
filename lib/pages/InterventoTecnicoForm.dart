...
class _InterventoTecnicoFormState extends State<InterventoTecnicoForm> {
  ...
  Future<void> getAllTipologie() async {
    try {
      final response =
          await http.get(Uri.parse('$ipaddress2/api/tipologiaIntervento'));
    ...
    try {
      if (_selectedTipologia != null) {
        final response = await http.get(Uri.parse(
            '$ipaddress2/api/categorieIntervento/tipologia/${_selectedTipologia!.id}'));
        if (response.statusCode == 200) {
          var jsonData = jsonDecode(utf8.decode(response.bodyBytes));
        ...
        child: SingleChildScrollView(  // Aggiunto SingleChildScrollView
          child: Column(
            ...
            children: [
              ...
              if(_dataOdierna != null)
                Text('${_dataOdierna!.day}/${_dataOdierna!.month}/${_dataOdierna!.year}'),
              SizedBox(
                width: 300,
              ...
              Row(
                ...
                children: [
                  ...
                  Text("Disponibilità orario appuntamento ".toUpperCase()),
                ],
              ),
              ...
              Row(
                children: [
                  Text('Assegna'.toUpperCase(), style: TextStyle(color: Colors.black)),
                  Checkbox(
                    value: _assegnaColleghi,
                    onChanged: (bool? value) {
                      setState(() {
                        _assegnaColleghi = value!;
                      });
                    },
                  ),
                ],
              ),
              if(_assegnaColleghi == true)
                Column(
        ...
        for(var image in pickedImages){
          if(image.path != null && image.path.isNotEmpty){
            ...
            var request = http.MultipartRequest(
                'POST',
                Uri.parse('$ipaddress2/api/immagine/${intervento.id}')
            );
            request.files.add(
      ...
      try{
        ...
        final response = await http.post(
          ...
          body: jsonEncode({
            ...
            'data': _dataOdierna != null ? _dataOdierna?.toIso8601String() : null,//_dataOdierna.toIso8601String(),
            ...
            'firma_cliente': null,
            'utente_apertura' : widget.userData.toMap(),
      ...
      try {
        ...
        response = await http.post(
          ...
          body: jsonEncode({
            ...
            'titolo' : _titoloController.text,
            'priorita' : prioritaString,
            'data': _dataOdierna != null ? _dataOdierna?.toIso8601String() : null,
            'data_apertura_intervento' : DateTime.now().toIso8601String(),
