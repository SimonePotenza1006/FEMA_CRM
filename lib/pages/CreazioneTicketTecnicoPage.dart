  ...
  Future<void> savePics() async {
    ...
    try {
      ...
      for (var image in pickedImages) {
        if (image.path.isNotEmpty) {
          ...
          var request = http.MultipartRequest(
            'POST',
            Uri.parse('$ipaddressProva2/api/immagine/ticket/${int.parse(ticket.id.toString())}'),
          );
          request.files.add(
      ...
      if (file.path != null && file.path.isNotEmpty) {
        ...
        var request = http.MultipartRequest(
          'POST',
          Uri.parse('$ipaddressProva2/api/immagine/ticketaudio/${int.parse(ticket.id!.toString())}'),
        );
        request.files.add(
          await http.MultipartFile.fromPath(
            'ticket', // Field name
