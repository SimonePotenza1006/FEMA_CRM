...
class _RegistroCassaPageState extends State<RegistroCassaPage> {
  ...
  Future<void> deleteMovimentazione(MovimentiModel movimento) async {
    try {
      final response = await http.delete(
        Uri.parse('$ipaddressProva2/api/movimenti/${movimento.id}'),
        headers: {'Content-Type': 'application/json'},
      );
      if (response.statusCode == 204) {
...
// Funzione di arrotondamento
  ...
  Future<void> getAllMovimentazioni() async {
    try {
      var apiUrl = Uri.parse('$ipaddressProva2/api/movimenti/ordered');
      var response = await http.get(apiUrl);
...
// Funzione helper per scaricare immagini dato l'ID del movimento
  Future<List<Uint8List>> fetchImagesByMovimentoId(int? movimentoId) async {
    final url = '$ipaddressProva2/api/immagine/movimenti/$movimentoId/images';
