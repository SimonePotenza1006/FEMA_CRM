...
class _HomeFormTecnicoNewPageState extends State<HomeFormTecnicoNewPage>{
  ...
  Future<void> saveIngresso() async{
    try{
      final response = await http.post(
        Uri.parse('$ipaddress2/api/ingresso'),
  ...
  Future<void> getAllTasks() async {
    try {
      ...
      http.Response response = await http.get(Uri.parse('$ipaddress2/api/task/utente/$userId'));
      if (response.statusCode == 200) {
        var responseData = json.decode(response.body);
        List<TaskModel> tasks = [];
        for (var item in responseData) {
          TaskModel task = TaskModel.fromJson(item);
          if (task.attivo == true && task.accettato == false && task.utentecreate!.id != widget.userData!.id) {
            tasks.add(task);
            print('trfg '+task.id.toString()+' '+task.utentecreate!.id.toString()+' '+widget.userData!.id.toString());
  ...
  void interventoVisualizzato(InterventoModel intervento) async{
    try{
        ...
        } } else
      {
        ...
        http.Response response2 = await http
            .get(Uri.parse('$ipaddress2/api/relazioneUtentiInterventi/interventoutente/'+intervento.id.toString()+'/'+widget.userData!.id.toString()));
        if (response2.statusCode == 200) {
          print('res st 200 ');
          var responseData2 = json.decode(response2.body);
          //print('rrdd2 '+responseData2.toString());
